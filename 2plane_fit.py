...
def main():
    ...
    while True:
        ...
        plane_safe, error_safe = plane_safe.ToF_RANSAC(points_safe, res=cfg.Sensor["resolution"])
        ...
        two_plane_visualization(fig, plane_obstacle, plane_safe, points_obstacle, points_safe)
        ...
        plt.show()
        print(f"Obstacle plane N: {plane_obstacle.N}, d: {plane_obstacle.d}, Error: {error_obstacle}.")
...
def test():
    ...
    while True:
        ...
        points_plane1 = np.array([[i, j, time_refine_distances[i, j]] for [i, j] in plane1_index])
        points_plane2 = np.array([[i, j, time_refine_distances[i, j]] for [i, j] in plane2_index])
        ## 5. Plane fitting
        ...
        plane1.ToF_visualization(fig, time_refine_distances, time_refine_sigma, cfg.Sensor["resolution"], cfg.Sensor["output_shape"])
        plt.show()
